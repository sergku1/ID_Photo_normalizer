...
"""  Full functional. Translated as I could"""

dir_path = os.path.dirname(os.path.realpath(__file__))  # to get the file path
# dir_path = os.path.dirname(os.path.abspath(sys.argv[0]))  # to get the file path
calculon = dir_path
image_dir = dir_path + imageD
info_path = dir_path + infoP
trash_dir = dir_path + trashD
t_emp = dir_path + TempD
global list_ed
...
def img_visual(self, g_ey_p, set_label_ok_p, img_path_p,
    ...
    if (name_suffix == '.jpeg') or (name_suffix == '.pdf'):  #
        ...
        if img_path_p != image_dir:  # Checking if input images are raw
            self.pixmap = QPixmap(img_file_full)  # Checking if input images are raw
            set_label_ok_p.setPixmap(self.pixmap)
        elif img_path_p == image_dir:  # Checking that the raw input images folder is visible
            # **************************Converting a picture for non-standard images
            lis_files = os.listdir(t_emp)  # View folder
            ...
            if (temp0 == 1) and not ((self.radioButton_19.isChecked()) and (
                ...
                img_file_temp = t_emp + '/temp' + str(temp_max) + '.jpeg'
                self.pixmap = QPixmap(img_file_temp)  # image setup and output
                set_label_ok_p.setPixmap(self.pixmap)
            else:  #
                img_file_temp = t_emp + '/temp.jpeg'
                if name_suffix == '.jpeg':  #
...
def list_dirs_method_1(self):  # outputs a list of full names of all clients
    list_dirs = os.listdir(info_path)  # Indicating which folder to look in
...
class MyWindow(QtWidgets.QMainWindow, calculon_photo_ui_1.Ui_MainWindow):
    def __init__(self, parent=None):
        ...
        add_hotkey('alt + x', QtWidgets.qApp.quit)
        self.pushButton_18.clicked.connect(QtWidgets.qApp.quit)  # Logout button event
        add_hotkey('alt + n', self.select_new)
        add_hotkey('alt + g', self.select_go)
        add_hotkey('alt + t', self.select_t)
        add_hotkey('alt + p', self.select_p)
        ...
        lis_files = os.listdir(t_emp)  #
        len_temp = len(lis_files)
        i = 0
        while i < len_temp:  #
            img_temp_full = t_emp + '/' + lis_files[i]
            os.remove(img_temp_full)  #
            i += 1
        '''Converting files to a standard '''
        lis_files = os.listdir(image_dir)  # View folder
        lis_files.sort()  # sort
        ...
        while i < len_files:  #
            ...
            img_file_full = image_dir + '/' + img_file_name  # From Img file type .jpg, jpe, JPG
            if (name_suffix == '.jpg') or (name_suffix == '.jpe') or (name_suffix == '.JPG'):  #
                name_suffix = '.jpeg'
                img_file_dest = image_dir + '/' + img_name + '.jpeg'  # To Img moving in .jpeg format
    ...
    def mousePressEvent(self, e):  # mouse button pressed
        if (868 <= e.x() <= 1588) and (32 <= e.y() <= 932) and (self.checkBox_18.isChecked()):  #
            ...
            lis_files = os.listdir(image_dir)  # Viewing image_dir
            lis_files.sort()  # sort
            img_file_name = lis_files[g_ey]  # file name take by its number from list
            name_suffix = '.' + img_file_name.split('.')[
                -1]  # divide by dot and select the file extension and correct the dot that disappeared in the process
            if (name_suffix == '.jpeg') or (name_suffix == '.pdf'):  #
                img_file_full = image_dir + '/' + img_file_name  # full path and file name
                img_file_dest = t_emp + '/' + img_file_name  #
                ...
                if d_key < 4:  #
                    if d_key == 0:  #
                        ...
                        img_file_temp = t_emp + '/temp' + str(d_key) + '.jpeg'
                        ...
                        img_file_temp = t_emp + '/temp' + str(d_key - 1) + '.jpeg'  #
                        img1 = cv2.imread(img_file_temp)
                        height, width, channel = img1.shape
    ...
    def text_ch(self,
        ...
        lis_files = os.listdir(image_dir)  # image_dir which folder to look in
        ...
        img_file_full = image_dir + '/' + img_file_name  # image_dir
        img1 = cv2.imread(img_file_full)
        height, width, channel = img1.shape
    ...
    def data_file_write_1(
        ...
        mypath_10 = info_path + '/' + text_str_13  # in which folder to create the directory
        if not os.path.exists(mypath_10):  # with checking not to overwrite the existing one
            os.makedirs(mypath_10, 0o777)
        my_file_name = info_path + '/' + text_str_13 + '/' + text_str_12  # path and name of the file to be written
        ...
        if not os.path.isfile(img_file_dest):  # Not that file in path
            ...
            self.label_46.setText(
                'Image=' + ' ' + str(self.lineEdit_22.text()) + ' ' + str(self.lineEdit_29.text()) + ' ' + str(
                    suffix) + '.jpeg' + ' ' + 'was moved at ' + info_path)
            img_file_dest = trash_dir + '/' + str(img_file_name)
            shutil.move(img_file_full, img_file_dest)
            # os.remove(img_file_full)
            list_dirs = os.listdir(info_path)  #
    ...
    def ed_graph(self):  # Launch Graphix mode
        if self.checkBox_18.isChecked():  #
            ...
            activation_txt(self)
            lis_files = os.listdir(image_dir)  # start testing -is the image right type and size
            ...
            i = 0
            while i < len_temp:  #
                img_file_temp = t_emp + '/' + lis_files[i]  #
        ...
        if (len_points == 4) or (len_points == 8):  # Protection from starts until the points are completely filled
            ...
            if len_temp > 7:  #
                ...
                while i < 8:  # If there are all 8 dots for ID, erase all 8 temp files
                    temp_name = t_emp + '/temp' + str(i) + '.jpeg'
                    os.remove(temp_name)
                    i += 1
            ...
            if (self.checkBox_18.isChecked()) and (self.radioButton_19.isChecked()):  # ID execute
                ...
                img_file_temp1 = t_emp + '/' + img_file_name1  #
                img_file_temp2 = t_emp + '/' + img_file_name2  #
                # *****************work with pdf
                ...
                # ********************output on display
                img_file_temp = t_emp + '/temp.jpeg'  # image made from 2 images for pass
    ...
    def click_previous(self):  #
        ...
        if temp0 == 1:  # Rollback at the point entry area
            ...
            self.lineEdit_113.setText(str(edit_points_d))
            img_temp = t_emp + '/temp' + str(d_key - 1) + '.jpeg'  #
            ...
            if d_key == 1:  #
                ...
                img_temp = t_emp + '/temp' + str(d_key - 2) + '.jpeg'  #
    ...
    def click_go_select(self):  # Go! on graphic editor
        ...
        if self.checkBox_18.isChecked():  #
            ...
            img_file_full = image_dir + '/' + name_file_jpg  #
            if not os.path.isfile(img_file_full):  #
                img_file_full = image_dir + '/' + name_file_pdf  #
                img_file_dest = trash_dir + '/' + name_file_pdf  #
            else:
                img_file_full = image_dir + '/' + name_file_jpg  #
                img_file_dest = trash_dir + '/' + name_file_jpg  #
            name_file = name_file_jpg
            shutil.move(img_file_full, img_file_dest)  # Send to trash with the ability to rewrite
            # os.remove(img_file_full)  # erased file in image_dir
            img_file_full = t_emp + '/' + name_file  #
            os.remove(img_file_full)  # erase same from Temp
            img_file_full = t_emp + '/temp.jpeg'  # This file will replace the original one in the folder image_dir
            img_file_dest = image_dir + '/' + name_file  #
    ...
    def list_iof_c(self, g_ez):  # function to compare the lineEdit with a list of Full names
        list_iof = os.listdir(info_path)  # which folder to look in
        list_iof.sort()  # sort
